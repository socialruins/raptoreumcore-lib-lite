...
Transaction.prototype.getInputsHash = function () {
  const inputBuffers = this.inputs.map(input => {
    const prevTxIdBuf = Buffer.from(input.prevTxId, 'hex');
    const voutBuf = Buffer.alloc(4);
    voutBuf.writeUInt32LE(input.outputIndex);
    const scriptBuf = Buffer.from(input.output.script, 'hex');
    const sequenceBuf = Buffer.alloc(4);
    sequenceBuf.writeUInt32LE(input.sequenceNumber);
    return Buffer.concat([prevTxIdBuf, voutBuf, scriptBuf, sequenceBuf]);
  });

  const inputsObject = {
    inputs: inputBuffers
  };
  const sortedInputsObject = sortObjectKeys(inputsObject);
  const inputsBuffer = Buffer.concat(sortedInputsObject.inputs);
