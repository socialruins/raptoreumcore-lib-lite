...
Transaction.prototype.getInputsHash = function () {
  ...
  //const inputs = this.inputs.map(input => input.prevTxId);
  //const inputsBuffer = Buffer.concat(inputs.map(input => Buffer.from(input, 'hex')));
  ...
  for (const input of this.inputs) {
    ...
    const scriptSigBuf = Buffer.from(input.scriptSig, 'hex');
    const sequenceBuf = Buffer.alloc(4);
    sequenceBuf.writeUInt32LE(input.sequence);
    const inputBuf = Buffer.concat([prevTxIdBuf, voutBuf, scriptSigBuf, sequenceBuf]);
    inputBuffers.push(inputBuf);
  }
  const inputsBuffer = Buffer.concat(inputBuffers);
  const inputsHash = crypto.createHash('sha256').update(inputsBuffer).digest();
  const doubleInputsHash = crypto.createHash('sha256').update(inputsHash).digest();
