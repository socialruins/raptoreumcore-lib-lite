...
Transaction.prototype._hasFeeError = function (opts, unspent) {
  ...
  if (this.isSpecialTransaction() && this.hasExtraPayload()) {
    ...
    var specialFeeSats = thisFee * 1e8;
    var maximumFee = Math.max(maxFeePerNormalTransaction, instantSendFee, specialFeeSats);
    if (unspent > maximumFee) {
      if (this._missingChange()) {
        return new errors.Transaction.ChangeAddressMissing(
    ...
    if (unspent < maximumFee) {
      return new errors.Transaction.FeeError.TooSmall(
        '(SpecialTx) expected more than ' + minimumFee + ' but got ' + unspent
      );
    }
