...
function sortObjectValues(obj) {
  ...
  if (Array.isArray(obj)) {
    return obj.map(sortObjectValues);
  }
  return Object.fromEntries(
    Object.entries(obj)
      .sort((a, b) => {
        if (typeof a[1] === 'object' && typeof b[1] === 'object') {
          return sortObjectValues(a[1]).toString().localeCompare(sortObjectValues(b[1]).toString());
        } else {
          return a[1].toString().localeCompare(b[1].toString());
        }
      })
      .map(([k, v]) => [k, sortObjectValues(v)])
  );
}
...
// Fee amount for a special (future) transaction, this can be changed via spork, used for estimation
Transaction.FUTURE_FEE = 100;
...
Transaction.prototype._hasFeeError = function (opts, unspent) {
  ...
  if (!opts.disableLargeFees) {
    ...
    if (this.isSpecialTransaction() && this.hasExtraPayload()) {
      ...
      if(payload.fee > 0) {
        var specialFeeSats = maxFeePerNormalTransaction + payload.fee * 1e8;
        maximumFee = Math.max(maxFeePerNormalTransaction, instantSendFee, specialFeeSats);
      }
    }
...
Transaction.prototype.getInputsHash = function () {
  ...
  for (const input of this.inputs) {
    const prevTxIdBuf = Buffer.from(input.prevTxId, 'hex');
    const voutBuf = Buffer.alloc(4);
    voutBuf.writeUInt32LE(input.outputIndex);
    const scriptLengthBuf = Buffer.alloc(1, 0);
    const scriptBuf = Buffer.alloc(0);
    const sequenceBuf = Buffer.alloc(4);
    sequenceBuf.writeUInt32LE(input.sequenceNumber);


    const inputBuf = Buffer.concat([prevTxIdBuf, voutBuf, scriptLengthBuf, scriptBuf, sequenceBuf]);
    inputBuffers.push(inputBuf);
  }

  const inputsBuffer = Buffer.concat(inputBuffers);
  return Hash.sha256sha256(inputsBuffer).toString('hex');
};
*/
...
Transaction.prototype.getInputsHash = function () {
  ...
  for (const input of this.inputs) {
    ...
    const voutBuf = Buffer.alloc(4);
    voutBuf.writeUInt32LE(input.outputIndex);
    const scriptSigLengthBuf = Buffer.alloc(1, 0);
    const scriptSigBuf = Buffer.alloc(0);
    const sequenceBuf = Buffer.alloc(4);
    sequenceBuf.writeUInt32LE(input.sequenceNumber);
    const inputBuf = Buffer.concat([prevTxIdBuf, voutBuf, scriptSigLengthBuf, scriptSigBuf, sequenceBuf]);
    inputBuffers.push(inputBuf);
  }
  const inputsBuffer = Buffer.concat(inputBuffers);
  const sortedInputsBuffer = sortObjectValues(inputsBuffer);
  const inputsHash = Hash.sha256(sortedInputsBuffer);
  const doubleInputsHash = Hash.sha256(inputsHash);
  return doubleInputsHash.toString('hex');
}
