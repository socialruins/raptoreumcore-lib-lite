...
Transaction.prototype._hasFeeError = function (opts, unspent) {
  ...
  if(this.type === registeredTransactionTypes.TRANSACTION_FUTURE || this.type === registeredTransactionTypes.TRANSACTION_NEW_ASSET) {
    var specialFee = this.getSpecialTxFee();
    var maximumFee = Math.max(maxFeePerNormalTransaction, instantSendFee, specialFee);
    if (unspent > maximumFee) {
      if (this._missingChange()) {
        return new errors.Transaction.ChangeAddressMissing(
          '(SpecialTx) Fee is too large and no change address was provided'
        );
      }
      return new errors.Transaction.FeeError.TooLarge(
        '(SpecialTx) expected less than ' + maximumFee + ' but got ' + unspent
      );
    }
    if (unspent < maximumFee) {
      return new errors.Transaction.FeeError.TooSmall(
        '(SpecialTx) expected more than ' + maximumFee + ' but got ' + unspent
      );
    }
...
Transaction.prototype._updateChangeOutput = function () {
  ...
  var fee = this.getFee() + this.getSpecialTxFee();
