...
Transaction.prototype.getInputsHash = function () {
  ...
  const testtx = {
    vin: [
      {
        prevTxId:
          "411b45ff7ae04943f4cdcb1797ff633890746e00ecbc2eee5ee68af4fcceb90e",
        outputIndex: 1,
      },
    ],
  };
  const sha256Hasher = configuration.crypto.createHash("sha256");
  for (const input of testtx.vin) {
    const prevTxIdBuf = Buffer.from(input.prevTxId, "hex").reverse();
    sha256Hasher.write(prevTxIdBuf);
    const voutBuf = Buffer.alloc(4);
    voutBuf.writeUInt32LE(input.outputIndex);
    const prevVout = Buffer.from(voutBuf, "hex").reverse();
    sha256Hasher.write(prevVout);
  }

  var hash = sha256Hasher.digest();
  const sha256Hasher2 = configuration.crypto.createHash("sha256");
  sha256Hasher2.write(hash);
  var hash2 = sha256Hasher2.digest();

  var inputHash = Buffer.from(hash2, "hex").reverse();
  return inputHash;
};
