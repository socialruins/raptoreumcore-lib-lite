...
FutureTxPayload.fromBuffer = function fromBuffer(rawPayload) {
  ...
  console.log('maturity:', payload.maturity);
  payload.lockTime = payloadBufferReader.readUInt32LE();
  console.log('lockTime:', payload.lockTime);
  payload.lockOutputIndex = payloadBufferReader.readUInt16LE();
  console.log('lockOutputIndex:', payload.lockOutputIndex);
  payload.fee = payloadBufferReader.readUInt16LE();
  console.log('fee:', payload.fee);
  payload.updatableByDestination = payloadBufferReader.readUInt8();
  console.log('updatableByDestination:', payload.updatableByDestination);
  payload.exChainType = payloadBufferReader.readUInt16LE();
  console.log('exChainType:', payload.exChainType);
  payload.externalPayoutAddress = payloadBufferReader.readVarLengthBuffer();
  console.log('externalPayoutAddress:', payload.externalPayoutAddress);
  payload.externalTxid = payloadBufferReader
    .read(HASH_SIZE)
    .reverse()
    .toString('hex');
  console.log('externalTxid:', payload.externalTxid);
  payload.externalConfirmations = payloadBufferReader.readUInt16LE();
  console.log('externalConfirmations:', payload.externalConfirmations);
  payload.inputsHash = payloadBufferReader
    .read(HASH_SIZE)
    .reverse()
    .toString('hex');
  console.log('inputsHash:', payload.inputsHash);
...
FutureTxPayload.prototype.toBuffer = function toBuffer(options) {
  ...
  var isExternalPayoutAddressHex = utils.isHexaString(this.externalPayoutAddress) ? "hex" : "utf8";
  ...
  payloadBufferWriter
    ...
    .write(Buffer.from(this.externalPayoutAddress, isExternalPayoutAddressHex))
