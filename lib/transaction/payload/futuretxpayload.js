 ...
 * @property {boolean} updatableByDestination - true to allow some information of this transaction to be change by lockOutput address
...
FutureTxPayload.fromBuffer = function fromBuffer(rawPayload) {
  ...
  var updatableByDestinationSize = payloadBufferReader.readVarintNum();
  payload.updatableByDestination = payloadBufferReader.read(updatableByDestinationSize);
  payload.exChainType = payloadBufferReader.readUInt16LE();
  var externalPayoutAddressSize = payloadBufferReader.readVarintNum();
  payload.externalPayoutAddress = payloadBufferReader
    .read(externalPayoutAddressSize);
...
FutureTxPayload.prototype.toBuffer = function toBuffer(options) {
  ...
  var isExternalPayoutAddressHex = utils.isHexaString(this.externalPayoutAddress);
  var setExternalPayoutAddress = isExternalPayoutAddressHex ? Buffer.from(this.externalPayoutAddress, 'hex') : this.externalPayoutAddress;
  ...
  payloadBufferWriter
    ...
    .write(this.updatableByDestination)
    .writeUInt16LE(this.exChainType)
    .write(setExternalPayoutAddress)
