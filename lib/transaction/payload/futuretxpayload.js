...
FutureTxPayload.fromBuffer = function fromBuffer(rawPayload) {
  ...
  var externalPayoutAddressSize = payloadBufferReader.readVarLengthBuffer();
  
  if(utils.isHexaString(payloadBufferReader.read(externalPayoutAddressSize).toString("hex"))) {
  ...
  console.log('externalPayoutAddressSize and externalPayoutAddress:', externalPayoutAddressSize, payload.externalPayoutAddress);
...
FutureTxPayload.prototype.toBuffer = function toBuffer(options) {
  ...
  payloadBufferWriter
    ...
    .write(isExternalPayoutAddressHex)
