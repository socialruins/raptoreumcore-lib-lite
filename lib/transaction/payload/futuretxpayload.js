...
FutureTxPayload.fromBuffer = function fromBuffer(rawPayload) {
  ...
  payload.updatableByDestination = payloadBufferReader.readUInt8();
...
FutureTxPayload.prototype.toBuffer = function toBuffer(options) {
  ...
  payloadBufferWriter
    ...
    console.log(payloadBufferWriter);
    */

    const payloadBuffer = Buffer.alloc(84);

    payloadBuffer.writeUInt16LE(this.version, 0);
    payloadBuffer.writeInt32LE(this.maturity, 2);
    payloadBuffer.writeInt32LE(this.lockTime, 6);
    payloadBuffer.writeUInt16LE(this.lockOutputIndex, 10);
    payloadBuffer.writeUInt16LE(this.fee, 12);
    payloadBuffer.writeUInt8(this.updatableByDestination === 1 ? 1 : 0, 14);
    payloadBuffer.writeUInt16LE(this.exChainType, 15);
    Buffer.from(this.externalPayoutAddress, "utf-8").copy(payloadBuffer, 17);
    Buffer.from(this.externalTxid, 'hex').reverse().copy(payloadBuffer, 49);
    payloadBuffer.writeUInt16LE(this.externalConfirmations, 65);
    Buffer.from(this.inputsHash, 'hex').reverse().copy(payloadBuffer, 67);
    
    //const encodedPayload = payloadBuffer.toString('hex');
